...
- ⚡ **現行デフォルト設定**: 5秒セグメント、20%オーバーラップ（1秒重複）、`top_k=5`、`threshold=0.1`
...
- `segment_duration`: セグメントの長さ（秒）（オプション、デフォルト: 5.0）
- `overlap`: オーバーラップ率（オプション、デフォルト: 0.2）

##### デフォルト設定について
**5秒セグメント（20%オーバーラップ＝1秒重複、hop 4秒）**を現行デフォルトにしています。

| 設定 | segment | overlap（比率） | 実際の重複 | hop | 30秒音声の推論回数 |
|------|---------|---------------|-----------|-----|-----------------|
| 旧設定 | 2秒 | 0.5 | 1秒 | 1秒 | 約29回 |
| **現行** | **5秒** | **0.2** | **1秒** | **4秒** | **約7回** |

2026-03-09 に旧設定（2秒/0.5）から変更。本番デプロイ後の確認で、検出精度はほぼ変わらず、レスポンスが大幅に改善されることを確認済み。overlap 0.2（実時間1秒の重複）により、セグメント境界をまたぐイベントの検出漏れも抑えています。
...
2. **spot_features**テーブル
   ...
   [
     ...
     {"time": 4.0, "events": [{"label": "Music", "score": 0.72}, ...]},
     {"time": 8.0, "events": [{"label": "Silence", "score": 0.91}, ...]},
     ...
   ]
   ```
